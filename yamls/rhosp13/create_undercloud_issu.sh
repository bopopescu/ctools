...
sshpass -p ${root_password} ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -l root ${undercloud_ip} " (
    ...
    #sudo curl -O -k https://cc-lab-satellite.englab.juniper.net/pub/katello-ca-consumer-latest.noarch.rpm
    #sudo rpm -Uvh katello-ca-consumer-latest.noarch.rpm
    #sudo subscription-manager register --activationkey=ak_release_test_r1909_rhel,ak_release_test_r1909_rhos  --org=ContrailCloudNFR 
    ...
    sudo subscription-manager attach --pool 8a4c58186d6dd239016dd8e590721e61
    sleep 10
...
sshpass -p ${stack_password} ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -l ${stack_user} ${undercloud_ip} " (
    ...
    sudo ip link add name vlan${vlan_1} link br-ctlplane type vlan id ${vlan_1}
    sudo ip addr add 10.1.0.254/24 dev vlan${vlan_1} 
    sudo ip link set dev vlan${vlan_1} up
    sudo ip link add name vlan${vlan_6} link br-ctlplane type vlan id ${vlan_6}
    sudo ip addr add 10.0.0.254/24 dev vlan${vlan_6}
    sudo ip link set dev vlan${vlan_6} up    
    echo vlan${vlan_2}
    echo vlan${vlan_2}
    echo ${vlan_2}
    echo vlan${vlan_2}
