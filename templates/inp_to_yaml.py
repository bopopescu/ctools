...
def change_server_dict():
    for clus in all_cluster_dict:
        ...
        for i in server_dict:
            ...
            for j in a:
                if project_uuid not in j and j != "public":
...
# Method for getting all floating ips of all the servers in the cluster

def get_all_fip_list():
    fixedip_to_floatingip_mapping_dict = {}
    ret_dict = {}
    project_uuid = general_params_dict["project_uuid"]
    a = subprocess.Popen('neutron floatingip-list --tenant_id %s -f json' %
...
def get_deployer():
    ...
    for clus in provision_5_0_dict:
        print provision_5_0_dict
        if "deployer" in provision_5_0_dict[clus]["deployment_config"]:
            print provision_5_0_dict[clus]["deployment_config"]["deployer"]
...
def create_yaml_file_for_5_0_provisioning():
    ...
    for clus in provision_5_0_dict:
	    ...
	    if "kolla" in provision_5_0_dict[clus]["deployment_config"]["deployments"]:
		final_prov_yaml_string = final_prov_yaml_string + "    kolla:\n"
		final_prov_yaml_string = final_prov_yaml_string + "      branch: %s\n" %provision_5_0_dict[clus]["deployment_config"]["deployments"]["kolla"]["branch"]
		final_prov_yaml_string = final_prov_yaml_string + "      registry: %s\n" % provision_5_0_dict[clus]["deployment_config"]["deployments"]["kolla"]["registry"]
		...
		final_prov_yaml_string = final_prov_yaml_string + "      registry: %s\n" %provision_5_0_dict[clus]["deployment_config"]["deployments"]["helm"]["registry"]
            if "openshift" in provision_5_0_dict[clus]["deployment_config"]["deployments"]:
                final_prov_yaml_string = final_prov_yaml_string + "    openshift:\n"
                final_prov_yaml_string = final_prov_yaml_string + "      branch: %s\n" %provision_5_0_dict[clus]["deployment_config"]["deployments"]["openshift"]["branch"]
                final_prov_yaml_string = final_prov_yaml_string + "      registry: %s\n" %provision_5_0_dict[clus]["deployment_config"]["deployments"]["openshift"]["registry"]
	else:
    	    final_prov_yaml_string = final_prov_yaml_string + "    kolla:\n"
    	    ...
    	    final_prov_yaml_string = final_prov_yaml_string + "    contrail:\n"
    	    final_prov_yaml_string = final_prov_yaml_string + "      branch: __BRANCH__\n"
    	    final_prov_yaml_string = final_prov_yaml_string + "      registry: ci-repo.englab.juniper.net:5000\n"
    for clus in provision_5_0_dict:
    ...
    final_prov_yaml_string = final_prov_yaml_string + "\ninstances:\n"
    if "public" in network_name_list:
        server_data=build_server_data()
    for clus in server_dict:
        for i in server_dict[clus]:
	    if server_dict[clus][i]["server_manager"] != "true":
	        ...
	        final_prov_yaml_string = final_prov_yaml_string + "    provider: bms\n"
                if "public" in network_name_list:
                    for k in range(len(server_data)):
                        if server_dict[clus][i]["name"] == server_data[k]["name"]:
                            ip_address = server_data[k]["management_ip"]
                else:
                    for j in server_dict[clus][i]["ip_address"]:
                        role = network_dict[j]["role"]
                        if role == "management":
	...
	if (("two_interface" in provision_5_0_dict[clus]["contrail_config"]) and (provision_5_0_dict[clus]["contrail_config"]["two_interface"] == "true")):
		...
		if network_dict[j]["role"] == "control-data":
            ...
            list_of_control_data_ip_of_openstack_nodes = []
	    list_of_control_data_ip_of_control_nodes = []
	    list_of_mgmt_ip_of_controller_nodes = []
	    list_of_control_data_ip_of_openstack_nodes = []
	    for j in server_dict[clus]:
		if server_dict[clus][j]["server_manager"] != "true":
		    if "control" in server_dict[clus][j]["roles"]:
				...
				list_of_control_data_ip_of_control_nodes.append(server_dict[clus][j]["ip_address"][k])
                    if "config" in server_dict[clus][j]["roles"]:
                        if "public" in network_name_list:
                            for k in range(len(server_data)):
                                if server_dict[clus][j]["name"] == server_data[k]["name"]:
                                    list_of_mgmt_ip_of_controller_nodes.append(server_data[k]["management_ip"])
                        else:
                            for k in server_dict[clus][j]["ip_address"]:
                                if network_dict[k]["role"] == "management":
                                    list_of_mgmt_ip_of_controller_nodes.append(server_dict[clus][j]["ip_address"][k])
		    if "openstack" in server_dict[clus][j]["roles"]:
		        for k in server_dict[clus][j]["ip_address"]:
			    if network_dict[k]["role"] == "control-data":
				list_of_control_data_ip_of_openstack_nodes.append(server_dict[clus][j]["ip_address"][k])
	    tmp_control_data_ip_string = ",".join(list_of_control_data_ip_of_control_nodes)
	    tmp_mgmt_controller_ip_string = ",".join(list_of_mgmt_ip_of_controller_nodes)
	    tmp_control_data_openstack_ip_string = ",".join(list_of_control_data_ip_of_openstack_nodes)
	    final_prov_yaml_string = final_prov_yaml_string + "  CONTROLLER_NODES: %s\n" %tmp_mgmt_controller_ip_string
	    final_prov_yaml_string = final_prov_yaml_string + "  OPENSTACK_NODES: %s\n" %tmp_control_data_openstack_ip_string
	    final_prov_yaml_string = final_prov_yaml_string + "  CONTROL_NODES: %s\n" %tmp_control_data_ip_string
